...
@app.post("/productos")
def crear_producto(producto: Producto, db: Session = Depends(get_db), usuario: str = Depends(obtener_usuario_actual)):
...
def actualizar_producto(producto_id: int, producto: Producto, db: Session = Depends(get_db), usuario: str = Depends(obtener_usuario_actual)):
    item = db.query(ProductoDB).filter(ProductoDB.id == producto_id).first()
    if item is None:
        raise HTTPException(status_code=404, detail="Producto no encontrado")
...
def eliminar_producto(producto_id: int, db: Session = Depends(get_db), usuario: str = Depends(obtener_usuario_actual)):
    item = db.query(ProductoDB).filter(ProductoDB.id == producto_id).first()
    if item is None:
        raise HTTPException(status_code=404, detail="Producto no encontrado")
    db.delete(item)
...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    if usuario_db is None or not verificar_password(form_data.password, usuario_db.password_hash):
